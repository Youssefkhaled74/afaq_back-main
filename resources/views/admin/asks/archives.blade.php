                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title mb-0">Asks Viwes</h5>
                        </div>
                        <div class="card-body">
                                        ...
                                        <div class="dataTables_scroll">
                                            <div class="dataTables_scrollBody" style="position: relative; overflow: auto; width: 100%">
                                                ...
                                                <div style="margin-top: 20px; font-weight: 600; font-size: 16px;">
                                                    Showing 1 to <span id="showItems"></span> of <span>{{App\Models\Ask::archive()->count()}}</span> entries
                                                </div>
                                                                ...
                                                                <ul class="pagination flex-wrap justify-content-center" style="align-items: center;">
                                                                    ...
                                                                    @if (!$asks->onFirstPage())
                                                                        <li class="page-item mt-1">
                                                                            <a class="page-link" href="{{ $asks->previousPageUrl() }}"
                                                                            aria-label="Previous">
                                                                    ...
                                                                    @for ($i = 1; $i <= $asks->lastPage(); $i++)
                                                                        <li class="page-item mt-1 {{ $i == $asks->currentPage() ? 'active' : '' }}">
                                                                            <a class="page-link" href="{{ $asks->url($i) }}"
                                                                            @if ($i == $asks->currentPage()) style="font-weight:bold;" @endif>
                                                                                {{ $i }}
                                                                            </a>
                                                                        </li>
                                                                    @endfor
                                                                    ...
                                                                    <!-- Next Button -->
                                                                    @if ($asks->hasMorePages())
                                                                        <li class="page-item mt-1">
                                                                            <a class="page-link" href="{{ $asks->nextPageUrl() }}" aria-label="Next">
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body text-center p-5">
                                                                <form role="form" action="{{url(route('admin/asks/back'))}}" method="get">
...
@section('script')
    ...
    <script>
        ...
        $(document).on('click', '#load_more_button', function() {
            var urlPath = `{{ route("admin/asks/pagination/archives")}}/${offset}/${limit}`;
        ...
        $(document).on('keyup', '.data_search', function() {
            var q = $(this).val();
            var urlPath = "{{ route('admin/asks/search/archives') }}";
            event.preventDefault();
